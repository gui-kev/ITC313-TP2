...
public:
	...
	void del_product_clt(std::string& title, std::string& nom_clt, std::string& prenom_clt); 		//méthode permettant de supprimer un produit au panier d'achat d'un client
	void qtt_product_clt(std::string& title, std::string& nom_clt, std::string& prenom_clt, int new_qtt);		//permet de modifier la quantité d'un produit du panier
	Product* return_product(std::string& title); 			//permet de renvoyer le produit pour l'ajouter au panier d'un client
